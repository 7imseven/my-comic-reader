...
class _VideoListPageState extends State<VideoListPage> {
  ...
  Future<void> _reload() async {
    final v = await _storage.getVideosByTag(widget.tag);
  ...
  Future<void> _renameVideo(VideoItem video) async {
    ...
    final newName = await showDialog<String>(context: context, builder: (ctx) => AlertDialog(
      ...
      content: TextField(controller: controller, decoration: const InputDecoration(border: OutlineInputBorder(), hintText: '输入新名称'), autofocus: true),
      actions: [TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('取消')), ElevatedButton(onPressed: () => Navigator.pop(ctx, controller.text.trim()), child: const Text('保存'))],
    ));
    if (newName != null && newName.isNotEmpty && newName != video.name) { await _storage.renameVideo(video.id, newName); await _reload(); }
  }

  Future<void> _deleteVideo(VideoItem video) async {
    final confirm = await showDialog<bool>(context: context, builder: (ctx) => AlertDialog(
      title: const Text('删除视频'), content: Text('确定删除「${video.name}」？此操作不可恢复。'),
    ...
    await _storage.deleteVideo(video.id); await _reload();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: Text('📁 ${widget.tag}'), backgroundColor: Colors.white, elevation: 0.5),
      body: _videos.isEmpty
          ...
          : ListView.builder(padding: const EdgeInsets.fromLTRB(16, 12, 16, 20), itemCount: _videos.length, itemBuilder: (_, index) => _buildVideoItem(_videos[index])),
    );
  }
  ...
  Widget _coverPlaceholder() {
    return Container(color: const Color(0xFFEEEEEE), child: const Center(child: Icon(Icons.play_arrow, color: Color(0xFFBBBBBB), size: 28)));
  }
}
