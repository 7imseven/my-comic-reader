...
class VideoStorageService {
  ...
  Future<void> _saveMeta() async {
    final text = json.encode(_videos.map((c) => c.toJson()).toList());
  ...
  /// Import a video file into the sandbox
  Future<int> importVideo(File sourceFile, List<String> tags) async {
    ...
    // Copy file using stream (handles large files)
    ...
    try {
      final thumbPath = await VideoThumbnail.thumbnailFile(
        video: destPath,
        thumbnailPath: '${_coversDir.path}/$id.jpg',
    ...
    final video = VideoItem(
      ...
      progress: 0,
      duration: 0, // will be updated when first played
      fileName: destName,
      fileSize: fileSize,
      tags: tags,
      hasCover: hasCover,
  ...
  Future<void> deleteVideo(int id) async {
    ...
    final filePath = '${_videosDir.path}/${video.fileName}';
  ...
  /// Get the video file path for playback
  ...
  Future<void> exportBackup(String outputPath) async {
    ...
    for (final video in _videos) {
      final dataPath = '${_videosDir.path}/${video.fileName}';
  ...
  Future<int> importBackup(String filePath) async {
    ...
    for (int i = 0; i < count; i++) {
      ...
      final video = VideoItem(
        id: id, name: name, addedAt: DateTime.now(),
        fileName: destName, fileSize: dataLen,
        tags: tags, hasCover: hasCover,
      );
